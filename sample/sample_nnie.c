...
/*ssd Localzier*/
static SAMPLE_SVP_NNIE_MODEL_S s_stLocModel = {0};
static SAMPLE_SVP_NNIE_PARAM_S s_stLocNnieParam = {0};
static SAMPLE_SVP_NNIE_SSD_SOFTWARE_PARAM_S s_stLocSoftwareParam = {0};
static SAMPLE_SVP_NNIE_CFG_S   stLocNnieCfg = {0};
/*ssd Recognizer*/
static SAMPLE_SVP_NNIE_MODEL_S s_stRecModel = {0};
static SAMPLE_SVP_NNIE_PARAM_S s_stRecNnieParam = {0};
static SAMPLE_SVP_NNIE_SSD_SOFTWARE_PARAM_S s_stRecSoftwareParam = {0};
static SAMPLE_SVP_NNIE_CFG_S   stRecNnieCfg = {0};
/*ssd Digit Model*/
static SAMPLE_SVP_NNIE_MODEL_S s_stDigModel = {0};
static SAMPLE_SVP_NNIE_PARAM_S s_stDigNnieParam = {0};
static SAMPLE_SVP_NNIE_SSD_SOFTWARE_PARAM_S s_stDigSoftwareParam = {0};
static SAMPLE_SVP_NNIE_CFG_S   stDigNnieCfg = {0};
/*yolov1 para*/
    ...
    SVP_BLOB_S *pstDstRoi, SVP_BLOB_S *pstClassRoiNum, HI_FLOAT f32PrintResultThresh)
{
    ...
    for (i = 1; i < u32ClassNum; i++){
        ...
        for (j = 0; j < (HI_U32)ps32ClassRoiNum[i]; j++)
        {
            ...
            fprintf(fp,"%d %d %d %d %d %f\n",i, s32XMin, s32YMin, s32XMax, s32YMax, f32Score);
            // Bbox 설정
            setBbox(boxNum++,i,s32XMin, s32YMin, s32XMax, s32YMax, f32Score,'y');
        }
...
/******************************************************************************
* function : print detection result For Loacalizer
******************************************************************************/
static HI_S32 SAMPLE_SVP_NNIE_Detection_PrintResult_Localizer(SVP_BLOB_S *pstDstScore,
    SVP_BLOB_S *pstDstRoi, SVP_BLOB_S *pstClassRoiNum, HI_FLOAT f32PrintResultThresh)
{
    HI_U32 i = 0, j = 0;
    HI_U32 u32RoiNumBias = 0;
    ...
    for (i = 1; i < u32ClassNum; i++){
        ...
        for (j = 0; j < (HI_U32)ps32ClassRoiNum[i]; j++)
        {
            ...
            fprintf(fp,"%d %d %d %d %d %f\n",i, s32XMin, s32YMin, s32XMax, s32YMax, f32Score);
            // Bbox 설정
            setBbox(boxNum++,i,s32XMin, s32YMin, s32XMax, s32YMax, f32Score,'l');
            // Bbox 인덱스
            //boxNum++;
        }
        u32RoiNumBias += ps32ClassRoiNum[i];
...
/******************************************************************************
* function : print detection result For Recognizer
******************************************************************************/
static HI_S32 SAMPLE_SVP_NNIE_Detection_PrintResult_Recognizer(SVP_BLOB_S *pstDstScore,
    SVP_BLOB_S *pstDstRoi, SVP_BLOB_S *pstClassRoiNum, HI_FLOAT f32PrintResultThresh)
{
    HI_U32 i = 0, j = 0;
    HI_U32 u32RoiNumBias = 0;
    ...
    for (i = 1; i < u32ClassNum; i++){
        ...
        for (j = 0; j < (HI_U32)ps32ClassRoiNum[i]; j++)
        {
            ...
            fprintf(fp,"%d %d %d %d %d %f\n",i, s32XMin, s32YMin, s32XMax, s32YMax, f32Score);
            // Bbox 설정
            setBbox(boxNum++,i,s32XMin, s32YMin, s32XMax, s32YMax, f32Score,'r');
            // Bbox 인덱스
            //boxNum++;
        }
        u32RoiNumBias += ps32ClassRoiNum[i];
    ...
    // l 박스 수 설정
    //setBboxSize(boxNum,'r');
	fprintf(fp,"==============\n");
    fclose(fp);
    return HI_SUCCESS;
}
/******************************************************************************
* function : print detection result For Digit Model
******************************************************************************/
static HI_S32 SAMPLE_SVP_NNIE_Detection_PrintResult_Digit_Model(SVP_BLOB_S *pstDstScore,
    SVP_BLOB_S *pstDstRoi, SVP_BLOB_S *pstClassRoiNum, HI_FLOAT f32PrintResultThresh)
{
    HI_U32 i = 0, j = 0;
    HI_U32 u32RoiNumBias = 0;
    ...
    for (i = 1; i < u32ClassNum; i++){
        ...
        for (j = 0; j < (HI_U32)ps32ClassRoiNum[i]; j++)
        {
            ...
            fprintf(fp,"%d %d %d %d %d %f\n",i, s32XMin, s32YMin, s32XMax, s32YMax, f32Score);
            // Bbox 설정
            setBbox(boxNum++,i,s32XMin, s32YMin, s32XMax, s32YMax, f32Score,'d');
            // Bbox 인덱스
            //boxNum++;
        }
        u32RoiNumBias += ps32ClassRoiNum[i];
...
{
    // 하드코딩되어있는 클래스 갯수 변수로 선언
    HI_U32 ClassNum = 88;
    // nms thresh hold값 설정
    HI_U32 NmsThresh = 0.5f;
    HI_U32 i = 0;
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32ClassNum = 0;
    HI_U32 u32TotalSize = 0;
    ...
    pstSoftWareParam->u32SoftMaxInHeight = ClassNum;
    pstSoftWareParam->au32SoftMaxInChn[0] = pstSoftWareParam->au32PriorBoxHeight[0]*pstSoftWareParam->au32PriorBoxHeight[0]*pstSoftWareParam->u32SoftMaxInHeight*4;
    pstSoftWareParam->au32SoftMaxInChn[1] = pstSoftWareParam->au32PriorBoxHeight[1]*pstSoftWareParam->au32PriorBoxHeight[1]*pstSoftWareParam->u32SoftMaxInHeight*6;
    pstSoftWareParam->au32SoftMaxInChn[2] = pstSoftWareParam->au32PriorBoxHeight[2]*pstSoftWareParam->au32PriorBoxHeight[2]*pstSoftWareParam->u32SoftMaxInHeight*6;
    pstSoftWareParam->au32SoftMaxInChn[3] = pstSoftWareParam->au32PriorBoxHeight[3]*pstSoftWareParam->au32PriorBoxHeight[3]*pstSoftWareParam->u32SoftMaxInHeight*6;
    pstSoftWareParam->au32SoftMaxInChn[4] = pstSoftWareParam->au32PriorBoxHeight[4]*pstSoftWareParam->au32PriorBoxHeight[4]*pstSoftWareParam->u32SoftMaxInHeight*4;
    pstSoftWareParam->au32SoftMaxInChn[5] = pstSoftWareParam->au32PriorBoxHeight[5]*pstSoftWareParam->au32PriorBoxHeight[5]*pstSoftWareParam->u32SoftMaxInHeight*4;
    
    pstSoftWareParam->u32ConcatNum = 6;
    pstSoftWareParam->u32SoftMaxOutWidth = 1;
    //pstSoftWareParam->u32SoftMaxOutHeight = 21;
    pstSoftWareParam->u32SoftMaxOutHeight = ClassNum;
    pstSoftWareParam->u32SoftMaxOutChn = 8732;

    /*Set DetectionOut Parameters*/
    pstSoftWareParam->u32ClassNum = ClassNum;
    //pstSoftWareParam->u32ClassNum = 21;
    pstSoftWareParam->u32TopK = 400;
    pstSoftWareParam->u32KeepTopK = 200;
    pstSoftWareParam->u32NmsThresh = (HI_U32)(NmsThresh*SAMPLE_SVP_NNIE_QUANT_BASE);
...
/******************************************************************************
* function : Ssd software para init For Localizer
******************************************************************************/
static HI_S32 SAMPLE_SVP_NNIE_Ssd_SoftwareInit_Localizer(SAMPLE_SVP_NNIE_CFG_S* pstCfg,
    SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam, SAMPLE_SVP_NNIE_SSD_SOFTWARE_PARAM_S* pstSoftWareParam)
{
    // 하드코딩되어있는 클래스 갯수 변수로 선언
    HI_U32 ClassNum = LOCALCLASSNUM;
    // nms thresh hold값 설정
    HI_U32 NmsThresh = LOCALNMSTHRESH;
    HI_U32 i = 0;
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32ClassNum = 0;
    HI_U32 u32TotalSize = 0;
...
/******************************************************************************
* function : Ssd software para init For Recognizer
******************************************************************************/
static HI_S32 SAMPLE_SVP_NNIE_Ssd_SoftwareInit_Recognizer(SAMPLE_SVP_NNIE_CFG_S* pstCfg,
    SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam, SAMPLE_SVP_NNIE_SSD_SOFTWARE_PARAM_S* pstSoftWareParam)
{
    // 하드코딩되어있는 클래스 갯수 변수로 선언
    HI_U32 ClassNum = RECOGCLASSNUM;
    // nms thresh hold값 설정
    HI_U32 NmsThresh = RECOGNMSTHRESH;
    HI_U32 i = 0;
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32ClassNum = 0;
    HI_U32 u32TotalSize = 0;
...
/******************************************************************************
* function : Ssd software para init For Digit Model
******************************************************************************/
static HI_S32 SAMPLE_SVP_NNIE_Ssd_SoftwareInit_Digit_Model(SAMPLE_SVP_NNIE_CFG_S* pstCfg,
    SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam, SAMPLE_SVP_NNIE_SSD_SOFTWARE_PARAM_S* pstSoftWareParam)
{
    // 하드코딩되어있는 클래스 갯수 변수로 선언
    HI_U32 ClassNum = DIGITMCLASSNUM;
    // nms thresh hold값 설정
    HI_U32 NmsThresh = DIGITMNMSTHRESH;
    HI_U32 i = 0;
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32ClassNum = 0;
    HI_U32 u32TotalSize = 0;
...
/******************************************************************************
* function : Ssd init Localizer
******************************************************************************/
static HI_S32 SAMPLE_SVP_NNIE_Ssd_ParamInit_Localizer(SAMPLE_SVP_NNIE_CFG_S* pstCfg,
    SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam, SAMPLE_SVP_NNIE_SSD_SOFTWARE_PARAM_S* pstSoftWareParam)
{
    HI_S32 s32Ret = HI_SUCCESS;
    /*init hardware para*/
    ...
    /*init software para*/
    s32Ret = SAMPLE_SVP_NNIE_Ssd_SoftwareInit_Localizer(pstCfg,pstNnieParam,
        pstSoftWareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,INIT_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error(%#x),SAMPLE_SVP_NNIE_Ssd_SoftwareInit_Localizer failed!\n",s32Ret);
...
/******************************************************************************
* function : Ssd init Recogizer
******************************************************************************/
static HI_S32 SAMPLE_SVP_NNIE_Ssd_ParamInit_Recognizer(SAMPLE_SVP_NNIE_CFG_S* pstCfg,
    SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam, SAMPLE_SVP_NNIE_SSD_SOFTWARE_PARAM_S* pstSoftWareParam)
{
    HI_S32 s32Ret = HI_SUCCESS;
    /*init hardware para*/
    ...
    /*init software para*/
    s32Ret = SAMPLE_SVP_NNIE_Ssd_SoftwareInit_Recognizer(pstCfg,pstNnieParam,
        pstSoftWareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,INIT_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error(%#x),SAMPLE_SVP_NNIE_Ssd_SoftwareInit_Recognizer failed!\n",s32Ret);
...
/******************************************************************************
* function : Ssd init Digit Model
******************************************************************************/
static HI_S32 SAMPLE_SVP_NNIE_Ssd_ParamInit_Digit_Model(SAMPLE_SVP_NNIE_CFG_S* pstCfg,
    SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam, SAMPLE_SVP_NNIE_SSD_SOFTWARE_PARAM_S* pstSoftWareParam)
{
    HI_S32 s32Ret = HI_SUCCESS;
    /*init hardware para*/
    ...
    /*init software para*/
    s32Ret = SAMPLE_SVP_NNIE_Ssd_SoftwareInit_Digit_Model(pstCfg,pstNnieParam,
        pstSoftWareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,INIT_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error(%#x),SAMPLE_SVP_NNIE_Ssd_SoftwareInit_Digit_Model failed!\n",s32Ret);
...
{
    HI_CHAR *pcSrcFile = "./data/nnie_image/rgb_planar/crop_2018_oct_g1_1047_874170.bgr";
    HI_CHAR *pcModelName = "./data/nnie_model/detection/r1_2_inst.wk";
    ...
    SAMPLE_COMM_SVP_CheckSysInit();
    e=clock();
    SAMPLE_SVP_TRACE_INFO("SSD System init : %f sec\n",(float)(e-s)/CLOCKS_PER_SEC);
    ...
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_COMM_SVP_NNIE_LoadModel failed!\n");
    e=clock();
    SAMPLE_SVP_TRACE_INFO("SSD Load Model : %f sec\n",(float)(e-s)/CLOCKS_PER_SEC);
    ...
    e=clock();
    SAMPLE_SVP_TRACE_INFO("SSD Inference : %f sec\n",(float)(e-s)/CLOCKS_PER_SEC);
    /*print result, this sample has 21 classes:
     class 0:background     class 1:plane           class 2:bicycle
     class 3:bird           class 4:boat            class 5:bottle
     class 6:bus            class 7:car             class 8:cat
...
void SAMPLE_SVP_NNIE_Ssd_Localizer(void)
{
    ...
    ROI.txt
    */
    //fputs("img dog_bike_car.jpg\n",fp);
    //HI_CHAR *pcSrcFile = "./data/nnie_image/rgb_planar/python/0000_17no9378.bgr";
    HI_CHAR *pcSrcFile = "./local_img.bgr";
    HI_CHAR *pcModelName = "./data/nnie_model/detection/localizer.wk";
    //HI_CHAR *pcModelName = "./data/nnie_model/detection/inst_ssd_cycle.wk";
    SAMPLE_SVP_TRACE_INFO("img : %s\n",pcSrcFile);
    SAMPLE_SVP_TRACE_INFO("model : %s\n",pcModelName);
    HI_U32 u32PicNum = 1;
    ...
    /*Set configuration parameter*/
    f32PrintResultThresh = LOCALTHRESH;
    stNnieCfg.pszPic= pcSrcFile;
    stNnieCfg.u32MaxInputNum = u32PicNum; //max input image num in each batch
    stNnieCfg.u32MaxRoiNum = 0;
    stNnieCfg.aenNnieCoreId[0] = SVP_NNIE_ID_0;//set NNIE core
    ...
    SAMPLE_COMM_SVP_CheckSysInit();
    e=clock();
    SAMPLE_SVP_TRACE_INFO("SSD System init : %f ms\n",1000*(float)(e-s)/CLOCKS_PER_SEC);
    ...
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_COMM_SVP_NNIE_LoadModel failed!\n");
    e=clock();
    SAMPLE_SVP_TRACE_INFO("SSD Load Model : %f ms\n",1000*(float)(e-s)/CLOCKS_PER_SEC);
    /*Ssd parameter initialization*/
    /*Ssd parameters are set in SAMPLE_SVP_NNIE_Ssd_SoftwareInit,
      if user has changed net struct, please make sure the parameter settings in
      SAMPLE_SVP_NNIE_Ssd_SoftwareInit function are correct*/
    ...
    s_stSsdNnieParam.pstModel = &s_stSsdModel.stModel;
    // Localizer 초기화 모델 변경
    s32Ret = SAMPLE_SVP_NNIE_Ssd_ParamInit_Localizer(&stNnieCfg,&s_stSsdNnieParam,&s_stSsdSoftwareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_Ssd_ParamInit_Localizer failed!\n");
    ...
    e=clock();
    SAMPLE_SVP_TRACE_INFO("SSD Inference : %f ms\n",1000*(float)(e-s)/CLOCKS_PER_SEC);
    /*software process*/
    /*if user has changed net struct, please make sure SAMPLE_SVP_NNIE_Ssd_GetResult
     function's input datas are correct*/
    SAMPLE_SVP_NNIE_PERF_STAT_BEGIN();
    ...
    SAMPLE_SVP_NNIE_PERF_STAT_SSD_GR_OP_TIME();
     ...
     SAMPLE_SVP_TRACE_INFO("Ssd result:\n");
     // L 모델 결과 출력
    (void)SAMPLE_SVP_NNIE_Detection_PrintResult_Localizer(&s_stSsdSoftwareParam.stDstScore,
        &s_stSsdSoftwareParam.stDstRoi, &s_stSsdSoftwareParam.stClassRoiNum,f32PrintResultThresh);
    SAMPLE_SVP_NNIE_PERF_STAT_END_LOOP();
...
void SAMPLE_SVP_NNIE_Ssd_Recognizer(void)
{
    ...
    HI_CHAR *pcModelName = "./data/nnie_model/detection/107_2_inst.wk";
    //HI_CHAR *pcModelName = "./data/nnie_model/detection/inst_ssd_cycle.wk";
    SAMPLE_SVP_TRACE_INFO("img : %s\n",pcSrcFile);
    SAMPLE_SVP_TRACE_INFO("model : %s\n",pcModelName);
    HI_U32 u32PicNum = 1;
    ...
    /*Set configuration parameter*/
    f32PrintResultThresh = RECOGTHRESH;
    stNnieCfg.pszPic= pcSrcFile;
    stNnieCfg.u32MaxInputNum = u32PicNum; //max input image num in each batch
    stNnieCfg.u32MaxRoiNum = 0;
    stNnieCfg.aenNnieCoreId[0] = SVP_NNIE_ID_0;//set NNIE core
    ...
    //SAMPLE_SVP_TRACE_INFO("Ssd parameter initialization!\n");
    s_stSsdNnieParam.pstModel = &s_stSsdModel.stModel;
    // Recognizer 초기화 모델 변경
    s32Ret = SAMPLE_SVP_NNIE_Ssd_ParamInit_Recognizer(&stNnieCfg,&s_stSsdNnieParam,&s_stSsdSoftwareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_Ssd_ParamInit_Recognizer failed!\n");
    ...
    /*print result, this sample has 21 classes:
     ...
     SAMPLE_SVP_TRACE_INFO("Ssd result:\n");
     // R 모델 결과 출력
    (void)SAMPLE_SVP_NNIE_Detection_PrintResult_Recognizer(&s_stSsdSoftwareParam.stDstScore,
        &s_stSsdSoftwareParam.stDstRoi, &s_stSsdSoftwareParam.stClassRoiNum,f32PrintResultThresh);
    SAMPLE_SVP_NNIE_PERF_STAT_END_LOOP();
...
void SAMPLE_SVP_NNIE_Ssd_Digit_Model(void)
{
    ...
    /*Set configuration parameter*/
    f32PrintResultThresh = DIGITMTHRESH;
    stNnieCfg.pszPic= pcSrcFile;
    stNnieCfg.u32MaxInputNum = u32PicNum; //max input image num in each batch
    stNnieCfg.u32MaxRoiNum = 0;
    stNnieCfg.aenNnieCoreId[0] = SVP_NNIE_ID_0;//set NNIE core
    ...
    s_stSsdNnieParam.pstModel = &s_stSsdModel.stModel;
    // Recognizer 초기화 모델 변경
    s32Ret = SAMPLE_SVP_NNIE_Ssd_ParamInit_Digit_Model(&stNnieCfg,&s_stSsdNnieParam,&s_stSsdSoftwareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_Ssd_ParamInit_Recognizer failed!\n");
    ...
    /*print result, this sample has 21 classes:
     ...
     SAMPLE_SVP_TRACE_INFO("Ssd result:\n");
     // R 모델 결과 출력
    (void)SAMPLE_SVP_NNIE_Detection_PrintResult_Digit_Model(&s_stSsdSoftwareParam.stDstScore,
        &s_stSsdSoftwareParam.stDstRoi, &s_stSsdSoftwareParam.stClassRoiNum,f32PrintResultThresh);
    SAMPLE_SVP_NNIE_PERF_STAT_END_LOOP();
...
void SSD_ONE_IMG(char* imgName){
    ...
    fp = fopen("ROI.txt", "w");
	fclose(fp);
    //char inputImg[100]="./data/nnie_image/rgb_planar/test/0002_78루4333.jpg";
    //char inputImg[100]=imgName;
    printf("Input Img : %s\n",imgName);
    makeLocalImg(imgName);
    SAMPLE_SVP_NNIE_Ssd_Localizer();
    // #2 l모델 과정을 거쳐서 나온 여러개 바운딩 박스중 박스 하나만을 검출.
	Bbox* box = getLocalResult();
    //Bbox* box;
    if(box==NULL){
        printf("l model failure\n");
        return;
    }
	// #3 검출한 박스를 크롭
    makeRecogImg(box,imgName);
    // #4 Digit만 꺼내오기
    /*
    SAMPLE_SVP_NNIE_Ssd_Digit_Model();
    char plateNumberDigitModel[MAXRESULTLEN] = "";
	getPlateStringDigitModel(plateNumberDigitModel);
    printf("digit : '%s'\n", plateNumberDigitModel);
    */
    /*
    SAMPLE_SVP_NNIE_Yolov3();
    char plateYOLO3[MAXRESULTLEN] = "";
	getPlateStringYOLO3(plateYOLO3);
    printf("YOLO3 '%s'\n", plateYOLO3);
    */
	// #5 r 모델을 거쳐서 나온 여러 바운딩 박스를 필터링을 거침.
    SAMPLE_SVP_NNIE_Ssd_Recognizer();
    // 결과값을 받아올 버퍼
	char plateNumber[MAXRESULTLEN] = "";
	getPlateString(plateNumber);
    printf("Recognize : '%s'\n", plateNumber);
}
void SSD_CONTROLLER(void){
    DIR* spDir;
    struct dirent *entry;
    char* dirName="./data/nnie_image/rgb_planar/test";
    //char* dirName=".";
    spDir=opendir(dirName);
    if(spDir==NULL){
        SAMPLE_SVP_TRACE_INFO("Test File open error!!\n");
        return 0;
    }
    /*Sys init*/
    SAMPLE_COMM_SVP_CheckSysInit();
    ...
    FILE *fp_csv=fopen("./result.csv","w");

    if(fp_csv==NULL){
        printf("result.csv open failed!\n");
        return;
    }
    fprintf(fp_csv,"idx,inference_time(localize),inference_time(recognize),result\n");
    fclose(fp_csv);

    while((entry=readdir(spDir))!=NULL){
        char imgName[MAXIMGNAMELEN]="";
        strcat(imgName,dirName);
        strcat(imgName,"/");
        strcat(imgName,entry->d_name);
        char* idx=strtok(entry->d_name,"_");
        if(!strcmp(entry->d_name,".")||!strcmp(entry->d_name,"..")){
            continue;
        }
        FILE *fp_csv=fopen("./result.csv","a");
        fprintf(fp_csv,"%s,",idx);
        fclose(fp_csv);

        printf("Input Img : %s\n",imgName);
        int ret=makeLocalImg(imgName);
        if(ret==0){
            continue;
        }
        // L 모델 inference
        ...
        //Bbox* box;
        if(box==NULL){
            printf("failed localize\n");
            continue;
        }
        // #3 검출한 박스를 크롭
        ret=makeRecogImg(box,imgName);
        if(ret==0){
            continue;
        }
        // Digit 모델 inference
        //DigitModel_Inference();
        // R 모델 inference
        Recognizer_Inference();
        char plateNumber[MAXRESULTLEN] = "";
        getPlateString(plateNumber);

        fp_csv=fopen("./result.csv","a");
        printf("plateNumber '%s'\n", plateNumber);
        fprintf(fp_csv,"%s\n",plateNumber);
        fclose(fp_csv);
    }
    SAMPLE_SVP_NNIE_Ssd_Deinit(&s_stLocNnieParam,&s_stLocSoftwareParam,&s_stLocModel);
    SAMPLE_SVP_NNIE_Ssd_Deinit(&s_stRecNnieParam,&s_stRecSoftwareParam,&s_stRecModel);
    SAMPLE_COMM_SVP_CheckSysExit();
}
void Localizer_Load(void){
    ...
    /*Set configuration parameter*/
    stLocNnieCfg.u32MaxInputNum = u32PicNum; //max input image num in each batch
    stLocNnieCfg.u32MaxRoiNum = 0;
    stLocNnieCfg.aenNnieCoreId[0] = SVP_NNIE_ID_0;//set NNIE core
    ...
    /*Ssd Load model*/
    //SAMPLE_SVP_TRACE_INFO("Ssd Load model!\n");
    s32Ret = SAMPLE_COMM_SVP_NNIE_LoadModel(pcModelName,&s_stLocModel);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_COMM_SVP_NNIE_LoadModel failed!\n");
    /*Ssd parameter initialization*/
    s_stLocNnieParam.pstModel = &s_stLocModel.stModel;
    // Localizer 초기화 모델 변경
    s32Ret = SAMPLE_SVP_NNIE_Ssd_ParamInit_Localizer(&stLocNnieCfg,&s_stLocNnieParam,&s_stLocSoftwareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_Ssd_ParamInit_Localizer failed!\n");
...
void Localizer_Inference(void){
    ...
    f32PrintResultThresh = LOCALTHRESH;
    stLocNnieCfg.pszPic= "./local_img.bgr";
    ...
    stInputDataIdx.u32SegIdx = 0;
    stInputDataIdx.u32NodeIdx = 0;
    s32Ret = SAMPLE_SVP_NNIE_FillSrcData(&stLocNnieCfg,&s_stLocNnieParam,&stInputDataIdx);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_FillSrcData failed!\n");

    /*NNIE process(process the 0-th segment)*/
    stProcSegIdx.u32SegIdx = 0;
    s32Ret = SAMPLE_SVP_NNIE_Forward(&s_stLocNnieParam,&stInputDataIdx,&stProcSegIdx,HI_TRUE);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_Forward failed!\n");
    SAMPLE_SVP_NNIE_PERF_STAT_SSD_FORWARD_SRC_FLUSH_TIME()
    SAMPLE_SVP_NNIE_PERF_STAT_SSD_FORWARD_PRE_DST_FLUSH_TIME()
    ...
    /*if user has changed net struct, please make sure SAMPLE_SVP_NNIE_Ssd_GetResult
     function's input datas are correct*/
    SAMPLE_SVP_NNIE_PERF_STAT_BEGIN();
    s32Ret = SAMPLE_SVP_NNIE_Ssd_GetResult(&s_stLocNnieParam,&s_stLocSoftwareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_Ssd_GetResult failed!\n");
    SAMPLE_SVP_NNIE_PERF_STAT_END();
    SAMPLE_SVP_NNIE_PERF_STAT_SSD_GR_OP_TIME();
    ...
    //SAMPLE_SVP_TRACE_INFO("Ssd result:\n");
     // L 모델 결과 출력
    (void)SAMPLE_SVP_NNIE_Detection_PrintResult_Localizer(&s_stLocSoftwareParam.stDstScore,
        &s_stLocSoftwareParam.stDstRoi, &s_stLocSoftwareParam.stClassRoiNum,f32PrintResultThresh);
    SAMPLE_SVP_NNIE_PERF_STAT_END_LOOP();

    SAMPLE_SVP_NNIE_PERF_STAT_SSD_PRINT();
    e=clock();
    FILE *fp_csv=fopen("./result.csv","a");
    float inf_time=1000*(float)(e-s)/CLOCKS_PER_SEC;
    inf_time=roundf(inf_time*100)/100;
    char str_inf_time[20];
    sprintf(str_inf_time,"%f",inf_time);
    fprintf(fp_csv,"%s,",str_inf_time);
    fclose(fp_csv);
...
void Recognizer_Load(void){
    HI_CHAR *pcModelName = "./data/nnie_model/detection/107_98_inst.wk";
    SAMPLE_SVP_TRACE_INFO("model : %s\n",pcModelName);
    HI_U32 u32PicNum = 1;
    //HI_FLOAT f32PrintResultThresh = 0.0f;
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    /*Set configuration parameter*/
    stRecNnieCfg.u32MaxInputNum = u32PicNum; //max input image num in each batch
    stRecNnieCfg.u32MaxRoiNum = 0;
    stRecNnieCfg.aenNnieCoreId[0] = SVP_NNIE_ID_0;//set NNIE core

    /*Ssd Load model*/
    //SAMPLE_SVP_TRACE_INFO("Ssd Load model!\n");
    s32Ret = SAMPLE_COMM_SVP_NNIE_LoadModel(pcModelName,&s_stRecModel);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_COMM_SVP_NNIE_LoadModel failed!\n");
    /*Ssd parameter initialization*/
    s_stRecNnieParam.pstModel = &s_stRecModel.stModel;
    // Localizer 초기화 모델 변경
    s32Ret = SAMPLE_SVP_NNIE_Ssd_ParamInit_Recognizer(&stRecNnieCfg,&s_stRecNnieParam,&s_stRecSoftwareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_Ssd_ParamInit_Recognizer failed!\n");
...
void Recognizer_Inference(void){
    ...
    f32PrintResultThresh = RECOGTHRESH;
    stRecNnieCfg.pszPic= "./recog_img.bgr";
    ...
    stInputDataIdx.u32SegIdx = 0;
    stInputDataIdx.u32NodeIdx = 0;
    s32Ret = SAMPLE_SVP_NNIE_FillSrcData(&stRecNnieCfg,&s_stRecNnieParam,&stInputDataIdx);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_FillSrcData failed!\n");

    /*NNIE process(process the 0-th segment)*/
    stProcSegIdx.u32SegIdx = 0;
    s32Ret = SAMPLE_SVP_NNIE_Forward(&s_stRecNnieParam,&stInputDataIdx,&stProcSegIdx,HI_TRUE);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_Forward failed!\n");
    SAMPLE_SVP_NNIE_PERF_STAT_SSD_FORWARD_SRC_FLUSH_TIME()
    SAMPLE_SVP_NNIE_PERF_STAT_SSD_FORWARD_PRE_DST_FLUSH_TIME()
    ...
    /*if user has changed net struct, please make sure SAMPLE_SVP_NNIE_Ssd_GetResult
     function's input datas are correct*/
    SAMPLE_SVP_NNIE_PERF_STAT_BEGIN();
    s32Ret = SAMPLE_SVP_NNIE_Ssd_GetResult(&s_stRecNnieParam,&s_stRecSoftwareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_Ssd_GetResult failed!\n");
    SAMPLE_SVP_NNIE_PERF_STAT_END();
    SAMPLE_SVP_NNIE_PERF_STAT_SSD_GR_OP_TIME();
    ...
    //SAMPLE_SVP_TRACE_INFO("Ssd result:\n");
     // L 모델 결과 출력
    (void)SAMPLE_SVP_NNIE_Detection_PrintResult_Recognizer(&s_stRecSoftwareParam.stDstScore,
        &s_stRecSoftwareParam.stDstRoi, &s_stRecSoftwareParam.stClassRoiNum,f32PrintResultThresh);
    SAMPLE_SVP_NNIE_PERF_STAT_END_LOOP();
...
void DigitModel_Load(void){
    HI_CHAR *pcModelName = "./data/nnie_model/detection/13_2_inst.wk";
    SAMPLE_SVP_TRACE_INFO("model : %s\n",pcModelName);
    HI_U32 u32PicNum = 1;
    //HI_FLOAT f32PrintResultThresh = 0.0f;
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    /*Set configuration parameter*/
    stDigNnieCfg.u32MaxInputNum = u32PicNum; //max input image num in each batch
    stDigNnieCfg.u32MaxRoiNum = 0;
    stDigNnieCfg.aenNnieCoreId[0] = SVP_NNIE_ID_0;//set NNIE core

    /*Ssd Load model*/
    //SAMPLE_SVP_TRACE_INFO("Ssd Load model!\n");
    s32Ret = SAMPLE_COMM_SVP_NNIE_LoadModel(pcModelName,&s_stDigModel);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_COMM_SVP_NNIE_LoadModel failed!\n");
    /*Ssd parameter initialization*/
    s_stDigNnieParam.pstModel = &s_stDigModel.stModel;
    // Localizer 초기화 모델 변경
    s32Ret = SAMPLE_SVP_NNIE_Ssd_ParamInit_Digit_Model(&stDigNnieCfg,&s_stDigNnieParam,&s_stDigSoftwareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_Ssd_ParamInit_Digognizer failed!\n");
...
void DigitModel_Inference(void){
    ...
    f32PrintResultThresh = DIGITMTHRESH;
    stDigNnieCfg.pszPic= "./recog_img.bgr";
    ...
    stInputDataIdx.u32SegIdx = 0;
    stInputDataIdx.u32NodeIdx = 0;
    s32Ret = SAMPLE_SVP_NNIE_FillSrcData(&stDigNnieCfg,&s_stDigNnieParam,&stInputDataIdx);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_FillSrcData failed!\n");

    /*NNIE process(process the 0-th segment)*/
    stProcSegIdx.u32SegIdx = 0;
    s32Ret = SAMPLE_SVP_NNIE_Forward(&s_stDigNnieParam,&stInputDataIdx,&stProcSegIdx,HI_TRUE);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_Forward failed!\n");
    SAMPLE_SVP_NNIE_PERF_STAT_SSD_FORWARD_SRC_FLUSH_TIME()
    SAMPLE_SVP_NNIE_PERF_STAT_SSD_FORWARD_PRE_DST_FLUSH_TIME()
    ...
    /*if user has changed net struct, please make sure SAMPLE_SVP_NNIE_Ssd_GetResult
     function's input datas are correct*/
    SAMPLE_SVP_NNIE_PERF_STAT_BEGIN();
    s32Ret = SAMPLE_SVP_NNIE_Ssd_GetResult(&s_stDigNnieParam,&s_stDigSoftwareParam);
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,SSD_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_SVP_NNIE_Ssd_GetResult failed!\n");
    SAMPLE_SVP_NNIE_PERF_STAT_END();
    SAMPLE_SVP_NNIE_PERF_STAT_SSD_GR_OP_TIME();
    ...
    //SAMPLE_SVP_TRACE_INFO("Ssd result:\n");
     // L 모델 결과 출력
    (void)SAMPLE_SVP_NNIE_Detection_PrintResult_Recognizer(&s_stDigSoftwareParam.stDstScore,
        &s_stDigSoftwareParam.stDstRoi, &s_stDigSoftwareParam.stClassRoiNum,f32PrintResultThresh);
    SAMPLE_SVP_NNIE_PERF_STAT_END_LOOP();
...
void SAMPLE_SVP_NNIE_Ssd_Localizer_Test(void)
{
    ...
    // ################################img 1
    stNnieCfg.pszPic= "./data/nnie_image/rgb_planar/python/0000_17no9378.bgr";
    SAMPLE_SVP_TRACE_INFO("img : %s\n",stNnieCfg.pszPic);
    ...
    // ################################img 2
    stNnieCfg.pszPic= "./data/nnie_image/rgb_planar/python/0001_69oo6365.bgr";
    SAMPLE_SVP_TRACE_INFO("img : %s\n",stNnieCfg.pszPic);
...
void SAMPLE_SVP_NNIE_Yolov1(void)
{
    ...
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,YOLOV1_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_COMM_SVP_NNIE_LoadModel failed!\n");
    e=clock();
    SAMPLE_SVP_TRACE_INFO("YOLO1 Load Model : %f sec\n",(float)(e-s)/CLOCKS_PER_SEC);
    ...
    e=clock();
    SAMPLE_SVP_TRACE_INFO("YOLO1 Inference : %f sec\n",(float)(e-s)/CLOCKS_PER_SEC);
    /*print result, this sample has 21 classes:
     class 0:background     class 1:plane           class 2:bicycle
     class 3:bird           class 4:boat            class 5:bottle
     class 6:bus            class 7:car             class 8:cat
...
void SAMPLE_SVP_NNIE_Yolov2(void)
{
    ...
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,YOLOV2_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_COMM_SVP_NNIE_LoadModel failed!\n");
    e=clock();
    SAMPLE_SVP_TRACE_INFO("YOLO2 Load Model : %f sec\n",(float)(e-s)/CLOCKS_PER_SEC);
    ...
    e=clock();
    SAMPLE_SVP_TRACE_INFO("YOLO2 Inference : %f sec\n",(float)(e-s)/CLOCKS_PER_SEC);
    ...
    SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam, SAMPLE_SVP_NNIE_YOLOV3_SOFTWARE_PARAM_S* pstSoftWareParam)
{
    ...
    pstSoftWareParam->u32ClassNum = YOLO3CLASSNUM-1; // 아래 u32ClassNum에서 1을 더해줌
    ...
    pstSoftWareParam->u32NmsThresh = (HI_U32)(YOLO3NMSTHRESH*SAMPLE_SVP_NNIE_QUANT_BASE);
...
void SAMPLE_SVP_NNIE_Yolov3(void)
{
    ...
    fp = fopen("ROI.txt", "a");
    fclose(fp);
    HI_CHAR *pcSrcFile = "./recog_416img.bgr";
    HI_CHAR *pcModelName = "./data/nnie_model/detection/yolo3_inst.wk";
    HI_U32 u32PicNum = 1;
    HI_FLOAT f32PrintResultThresh = 0.0f;
    HI_S32 s32Ret = HI_SUCCESS;
    SAMPLE_SVP_NNIE_CFG_S   stNnieCfg = {0};
    ...
    /*Set configuration parameter*/
    f32PrintResultThresh = YOLO3THRESH;
    stNnieCfg.pszPic= pcSrcFile;
    stNnieCfg.u32MaxInputNum = u32PicNum; //max input image num in each batch
    stNnieCfg.u32MaxRoiNum = 0;
    stNnieCfg.aenNnieCoreId[0] = SVP_NNIE_ID_0;//set NNIE core
    ...
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret,YOLOV3_FAIL_0,SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SAMPLE_COMM_SVP_NNIE_LoadModel failed!\n");
    e=clock();
    SAMPLE_SVP_TRACE_INFO("YOLO3 Load Model : %f sec\n",(float)(e-s)/CLOCKS_PER_SEC);
    ...
    e=clock();
    SAMPLE_SVP_TRACE_INFO("YOLO3 Inference : %f sec\n",(float)(e-s)/CLOCKS_PER_SEC);
